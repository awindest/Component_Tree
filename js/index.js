...
// http://dh.obdurodon.org/coordinate-tutorial.xhtml
    ...
    const promises = [
      // Talend Studio 7.3 data:
      // d3.json('data/ComponentDescriptions.json'),
      // d3.csv('data/TalendComponents.csv'),
      // d3.json('data/translations.json')
      d3.json('data/ComponentDescriptionsV8-2023.json'),
      d3.csv('data/TalendComponentsV8-2023.csv'),
      d3.json('data/translations.json'),
      d3.json('data/ComponentIconsV8-2023.json')
    ]
    ...
    // always check your data with console.log(data) otherwise things go screwy quickly.
      ...
      searchData = Object.keys(description)  // basically all of the names of the components
      ...
      function getIconReference(d) { //returns reference to the icon file
        const componentName = getComponentName(d)
        return `img/${iconData[componentName]}`
      }
